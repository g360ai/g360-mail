...
app.post('/api/templates', auth, adminOnly, async (req, res) => {
  try {
    ...
    const r = await insert('templates', { 
      name: req.body.name, 
...
app.put('/api/templates/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    // category_id sadece değer varsa veya açıkça null yapılacaksa ekle
    if (req.body.category_id !== undefined) {
      updateData.category_id = categoryId;
    }
    const r = await update('templates', req.params.id, updateData);
...
app.delete('/api/templates/:id', auth, adminOnly, async (req, res) => { 
  try {
    const r = await update('templates', req.params.id, { is_active: false, is_deleted: true }); 
    r ? res.json({ ok: true }) : res.status(500).json({ error: 'Taslak silinemedi' });
  } catch (err) {
