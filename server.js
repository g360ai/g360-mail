...
app.put('/api/users/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    // allowed_cities'i düzgün şekilde kaydet (TEXT[] için)
    if (req.body.allowed_cities !== undefined) {
      // Array olarak gönderilmişse direkt kullan, değilse boş array
      const cities = Array.isArray(req.body.allowed_cities) ? req.body.allowed_cities : [];
      updates.allowed_cities = cities;
      console.log('Saving allowed_cities:', cities);
    }
    ...
    console.log('User updated:', updated?.id, 'allowed_cities:', updated?.allowed_cities);
...
app.get('/api/customers', auth, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'sales') {
      ...
      // Bölge Yöneticisi: Ekip + İzin verilen şehirler
      let allowedCities = req.user.allowed_cities || [];
      
      // PostgreSQL TEXT[] array'i Supabase'den array olarak gelmeli
      // Ama string olarak gelirse parse et
      if (typeof allowedCities === 'string') {
        try { 
          // "{city1,city2}" formatından parse et
          if (allowedCities.startsWith('{') && allowedCities.endsWith('}')) {
            allowedCities = allowedCities.slice(1, -1).split(',').map(s => s.trim());
          } else {
            allowedCities = JSON.parse(allowedCities); 
          }
        } catch(e) { 
          allowedCities = []; 
        }
      ...
      const teamUserIds = allUsers.filter(u => u.team_id === req.user.team_id).map(u => u.id);
      
      console.log('=== MANAGER FILTER DEBUG ===');
      console.log('User:', req.user.full_name);
      console.log('Team ID:', req.user.team_id);
      console.log('Allowed cities (raw):', req.user.allowed_cities);
      console.log('Allowed cities (parsed):', allowedCities);
      console.log('Team User IDs:', teamUserIds);
      
      filtered = filtered.filter(c => {
        // Kendi ekibinin kayıtları
        if (teamUserIds.includes(c.user_id)) {
          return true;
        }
        
        // İzin verilen şehirlerdeki TÜM kayıtlar
        if (allowedCities.length > 0 && c.city) {
          const customerCity = c.city.toLowerCase().trim();
          const match = allowedCities.some(city => {
            if (!city) return false;
            const allowedCity = city.toLowerCase().trim();
            // Tam eşleşme veya içerme kontrolü
            return customerCity === allowedCity || 
                   customerCity.includes(allowedCity) || 
                   allowedCity.includes(customerCity);
          });
          if (match) {
            console.log('City match:', c.business_name, '-', c.city);
          }
          return match;
        }
        return false;
