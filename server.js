...
const STATUSES = [
  { id: 1, name: 'Müşteri Bilgilendirildi', color: '#f59e0b' },
  { id: 2, name: 'Çekim Tarihi Belirlendi', color: '#f97316' },
  { id: 3, name: 'Çekim Yapıldı', color: '#8b5cf6' },
  { id: 4, name: 'Müşteri Çekimi Onayladı', color: '#3b82f6' },
  { id: 5, name: 'İlgilenmiyor', color: '#ef4444' },
  { id: 6, name: 'Tamamlandı', color: '#10b981' }
];
...
async function initData() {
  ...
  if (teams.length === 0) {
    for (const name of ['Ankara', 'Istanbul', 'Izmir', 'Antalya']) {
  ...
  if (templates.length === 0) {
    await insert('templates', {
      ...
      body: '<p>Sayın <strong>{{isim}}</strong>,</p><p><strong>{{firma}}</strong> için Google Street View 360° iç mekân çekim teklifimizi sunarız.</p><p>💼 <strong>Fiyatlandırma:</strong> Sahne başı <strong>{{fiyat}} ₺</strong></p><p>İş birliği için teşekkür ederiz.</p>',
  ...
  if (!sig) {
    await setSetting('signature', '<p style="margin-top:20px;padding-top:15px;border-top:1px solid #ddd;color:#666;">Saygılarımızla,<br><strong style="color:#0d9488;">G360 AI Ekibi</strong></p>');
  }
}
...
app.post('/api/templates', auth, adminOnly, async (req, res) => {
  ...
  res.json(await insert('templates', { name: req.body.name, subject: req.body.subject, body: req.body.body, variables, is_active: true }));
});

app.put('/api/templates/:id', auth, adminOnly, async (req, res) => {
  const matches = (req.body.subject + ' ' + req.body.body).match(/\{\{(\w+)\}\}/g) || [];
  const variables = [...new Set(matches.map(m => m.slice(2, -2)))];
  res.json(await update('templates', req.params.id, { name: req.body.name, subject: req.body.subject, body: req.body.body, variables }));
});
...
app.patch('/api/customers/:id/status', auth, async (req, res) => {
  ...
  res.json(await update('customers', req.params.id, { status: newStatus }));
});
...
app.get('/api/stats', auth, async (req, res) => {
  ...
  res.json({
    ...
    todayRevenue: completed.filter(c => new Date(c.created_at) >= today).reduce((s, c) => s + (c.price || 0), 0),
    weekRevenue: completed.filter(c => new Date(c.created_at) >= weekAgo).reduce((s, c) => s + (c.price || 0), 0),
    monthRevenue: completed.filter(c => new Date(c.created_at) >= monthAgo).reduce((s, c) => s + (c.price || 0), 0),
...
app.get('/health', (req, res) => res.json({ ok: true, v: '5.0', db: !!supabase }));
...
app.listen(PORT, async () => {
  console.log('================================');
  console.log('G360 CRM + Mail v5.0');
  console.log('Port:', PORT);
  console.log('Supabase:', supabase ? 'Connected' : 'NOT SET');
  console.log('Resend:', RESEND_API_KEY ? 'OK' : 'NOT SET');
  console.log('Google Maps:', GOOGLE_MAPS_API_KEY ? 'OK' : 'NOT SET');
  console.log('================================');
  await initData();
});
