...
app.get('/api/customers', auth, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'sales') {
      ...
      // Bölge Yöneticisi: Ekip kayıtları + İzin verilen şehirler
      const users = await getAll('users');
      const teamUserIds = users.filter(u => u.team_id === req.user.team_id).map(u => u.id);
      const allowedCities = req.user.allowed_cities || [];
      
      if (allowedCities.length > 0) {
        // Ekip kayıtları VEYA izin verilen şehirlerdeki kayıtlar
        ...
        // Filter: team users OR allowed cities
        const filtered = (allData || []).filter(c => 
          teamUserIds.includes(c.user_id) || 
          allowedCities.some(city => c.city && c.city.toLowerCase().includes(city.toLowerCase()))
        );
        
        const usersData = await getAll('users');
        return res.json(filtered.map(c => ({ 
          ...c, 
          user_name: usersData.find(u => u.id === c.user_id)?.full_name,
          user_photo: usersData.find(u => u.id === c.user_id)?.profile_photo,
          category: CATEGORIES.find(cat => cat.id === c.category_id), 
          status_info: STATUSES.find(s => s.id === c.status) 
        })));
      } else if (teamUserIds.length) {
...
// Silinen müşteriler (sadece admin ve müdür)
app.get('/api/customers/deleted', auth, supervisorOrAdmin, async (req, res) => {
  if (!supabase) return res.json([]);
  try {
