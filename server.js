...
app.get('/api/customers', auth, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'sales') {
      ...
      console.log('MANAGER FILTER - Is Array:', Array.isArray(currentUser?.allowed_cities));
      
      // allowed_cities'i al ve parse et
      let allowedCities = [];
      ...
      if (rawCities) {
        if (Array.isArray(rawCities)) {
          ...
          if (rawCities.startsWith('{') && rawCities.endsWith('}')) {
            // PostgreSQL array format: {city1,city2}
            allowedCities = rawCities.slice(1, -1).split(',').map(s => s.trim().replace(/"/g, ''));
          } else if (rawCities.startsWith('[')) {
            // JSON array format
            try { allowedCities = JSON.parse(rawCities); } catch(e) { allowedCities = []; }
          } else {
            // Tek şehir
            allowedCities = [rawCities];
          }
      ...
      console.log('MANAGER FILTER - Customer cities:', customerCities.slice(0, 10));
      ...
      filtered = filtered.filter(c => {
        ...
        if (allowedCities.length > 0 && c.city) {
          ...
          const match = allowedCities.some(city => {
            ...
            if (isMatch) {
              console.log('MATCH FOUND:', c.business_name, '-', c.city, '<->', city);
            }
            return isMatch;
          });
          return match;
