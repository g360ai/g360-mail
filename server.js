...
app.put('/api/users/:id', auth, adminOnly, async (req, res) => {
  ...
  if (req.body.allowed_cities !== undefined) updates.allowed_cities = req.body.allowed_cities;
...
app.put('/api/profile/photo', auth, async (req, res) => {
  try {
    await update('users', req.user.id, { profile_photo: req.body.profile_photo || null });
    res.json({ ok: true });
  } catch (err) {
...
app.get('/api/customers', auth, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === 'sales') {
      ...
      // Bölge Yöneticisi: Şehir bazlı filtreleme
      // allowed_cities array'ini al
      let allowedCities = req.user.allowed_cities || [];
      // Eğer string ise parse et
      if (typeof allowedCities === 'string') {
        try { allowedCities = JSON.parse(allowedCities); } catch(e) { allowedCities = []; }
      }
      const teamUserIds = allUsers.filter(u => u.team_id === req.user.team_id).map(u => u.id);
      
      console.log('Manager filter - User:', req.user.full_name, 'Allowed cities:', allowedCities, 'Team IDs:', teamUserIds);
      
      filtered = filtered.filter(c => {
        // Kendi ekibinin kayıtları
        if (teamUserIds.includes(c.user_id)) return true;
        // İzin verilen şehirlerdeki TÜM kayıtlar (geçmiş dahil)
