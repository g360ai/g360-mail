...
const CATEGORIES = [
  ...
  // Eski CRM'den gelen ek kategoriler
  { id: 23, name: 'Emlak' }, { id: 24, name: 'Hastane' }, { id: 25, name: 'Organize Sanayi' },
  { id: 26, name: 'Diş Kliniği' }, { id: 27, name: 'Veteriner' }, { id: 28, name: 'Fabrika' },
  { id: 29, name: 'Showroom' }, { id: 30, name: 'Okul' }, { id: 31, name: 'Kreş' },
  { id: 32, name: 'Dil Kursu' }, { id: 33, name: 'Sürücü Kursu' }, { id: 34, name: 'Müze' },
  { id: 35, name: 'Galeri' }, { id: 36, name: 'Kuaför' }, { id: 37, name: 'Berber' },
  { id: 38, name: 'Spa' }, { id: 39, name: 'Hamam' }, { id: 40, name: 'Ev Tekstili' },
  { id: 41, name: 'Halı' }, { id: 42, name: 'Cafe & Restaurant' }, { id: 43, name: 'Butik Otel' },
  { id: 44, name: 'Pansiyon' }
];

// Eski CRM status mapping
const STATUS_MAPPING = {
  'created': 1,                      // Müşteri Bilgilendirildi
  'customer_information_sent': 1,    // Müşteri Bilgilendirildi
  'shoot_date_selected': 2,          // Çekim Tarihi Belirlendi
  'shooting_was_done': 3,            // Çekim Yapıldı
  'shoot_combined': 3,               // Çekim Yapıldı
  'customer_shoot_approved': 4,      // Müşteri Çekimi Onayladı
  'customer_confirmed': 4,           // Müşteri Çekimi Onayladı
  'not_interested': 5,               // İlgilenmiyor
  'customer_shoot_rejected': 5,      // İlgilenmiyor
  'completed': 6,                    // Tamamlandı
  'shoot_published': 6,              // Tamamlandı
  'payment_approved': 6,             // Tamamlandı
  'payment_sent': 6                  // Tamamlandı
};
...
async function update(table, id, updates) {
  if (!supabase) return null;
  try {
    // updated_at'i sadece customers tablosu için ekle (users tablosunda yok)
    if (table === 'customers') {
...
function auth(req, res, next) {
  ...
  jwt.verify(t, JWT_SECRET, async (e, u) => {
    if (e) return res.status(403).json({ error: 'Geçersiz token' });
    // Veritabanından güncel user bilgilerini çek (allowed_cities için)
    try {
      const freshUser = await getById('users', u.id);
      if (freshUser) {
        req.user = { ...u, ...freshUser, password: undefined };
      } else {
        req.user = u;
      }
    } catch (err) {
...
function supervisorOrAdmin(req, res, next) {
  if (req.user.role !== 'admin' && req.user.role !== 'supervisor') return res.status(403).json({ error: 'Yetki yok' });
  next();
}
...
app.get('/api/users', auth, async (req, res) => {
  ...
  // supervisor tüm kullanıcıları görebilir ama admin değil
...
app.put('/api/users/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    // allowed_cities'i array olarak kaydet (TEXT[] kolonu için)
    if (req.body.allowed_cities !== undefined) {
      const cities = Array.isArray(req.body.allowed_cities) ? req.body.allowed_cities : [];
      // Temizle ve array olarak kaydet
      updates.allowed_cities = cities.filter(c => c && String(c).trim()).map(c => String(c).trim());
      console.log('Saving allowed_cities as array:', updates.allowed_cities);
    }
    
    if (req.body.profile_photo !== undefined) updates.profile_photo = req.body.profile_photo;
    
    console.log('Updates object:', JSON.stringify(updates, null, 2));
    
    const updated = await update('users', req.params.id, updates);
    console.log('User updated result:', updated?.id, 'allowed_cities:', updated?.allowed_cities);
    console.log('=== END USER UPDATE ===');
    
    res.json({ ok: true, user: updated ? { ...updated, password: undefined } : null });
  } catch (err) {
...
// Profil fotoğrafı güncelleme (kendi profili için)
app.put('/api/profile/photo', auth, async (req, res) => {
  try {
    const updated = await update('users', req.user.id, { profile_photo: req.body.profile_photo || null });
    if (updated) {
      // Güncel user bilgisini döndür
      const teams = await getAll('teams');
      res.json({ 
        ok: true, 
        user: { 
          ...updated, 
          password: undefined, 
          team_name: teams.find(t => t.id === updated.team_id)?.name 
        } 
      });
    } else {
...
app.get('/api/templates', auth, async (req, res) => {
  try {
    const allTemplates = await getAll('templates');
    // is_active true olanları filtrele (veya is_active undefined ise dahil et)
    const templates = allTemplates.filter(t => t.is_active !== false);
    // Kategori bilgisini ekle ve sort_order'a göre sırala
    const result = templates.map(t => ({
      ...t,
      category: CATEGORIES.find(c => c.id === t.category_id)
    }));
    // Önce kategori'ye göre, sonra sort_order'a göre sırala
    result.sort((a, b) => {
      // Önce category_id'ye göre (null olanlar sona)
      const catA = a.category_id || 9999;
      const catB = b.category_id || 9999;
      if (catA !== catB) return catA - catB;
      // Sonra sort_order'a göre
      return (a.sort_order || 0) - (b.sort_order || 0);
    });
    res.json(result);
  } catch (err) {
    console.error('Templates GET error:', err);
    res.status(500).json({ error: 'Taslaklar yüklenemedi' });
  }
});
...
app.post('/api/templates', auth, adminOnly, async (req, res) => {
  try {
    ...
    // category_id: boş string veya undefined ise null yap
    const categoryId = req.body.category_id && req.body.category_id !== '' ? parseInt(req.body.category_id) : null;
    ...
    // is_deleted olmadan insert dene
    ...
    if (error) {
      ...
      return res.status(500).json({ error: 'Taslak oluşturulamadı: ' + error.message });
    }
    res.json(data);
  } catch (err) {
    console.error('Template POST error:', err);
    res.status(500).json({ error: 'Taslak oluşturulamadı: ' + err.message });
  }
});
...
app.put('/api/templates/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('templates').update(updateData).eq('id', req.params.id).select().single();
    
    if (error) {
      console.error('Template PUT error:', error);
      return res.status(500).json({ error: 'Taslak güncellenemedi: ' + error.message });
    }
    res.json(data);
  } catch (err) {
    console.error('Template PUT error:', err);
    res.status(500).json({ error: 'Taslak güncellenemedi: ' + err.message });
  }
});

app.delete('/api/templates/:id', auth, adminOnly, async (req, res) => { 
  try {
    // Sadece is_active false yap
    ...
    if (error) {
      ...
      return res.status(500).json({ error: 'Taslak silinemedi: ' + error.message });
    }
    res.json({ ok: true });
  } catch (err) {
    console.error('Template DELETE error:', err);
    res.status(500).json({ error: 'Taslak silinemedi: ' + err.message });
  }
});

// Template sıralama endpoint'i
app.put('/api/templates/reorder', auth, adminOnly, async (req, res) => {
  try {
    const { orders } = req.body; // [{id: 'xxx', sort_order: 1}, ...]
    if (!Array.isArray(orders)) return res.status(400).json({ error: 'orders array gerekli' });
    
    for (const item of orders) {
      await supabase.from('templates').update({ sort_order: item.sort_order }).eq('id', item.id);
    }
    res.json({ ok: true });
  } catch (err) {
...
app.post('/api/import/customers', auth, adminOnly, async (req, res) => {
  try {
    ...
    for (const c of customers) {
      try {
        ...
        if (c.status && STATUS_MAPPING[c.status]) {
          statusId = STATUS_MAPPING[c.status];
        }
        
        // Category mapping - eski ID'leri kullan
        let categoryId = c.category_id || 1;
        if (categoryId > 44) categoryId = 1;
        
        // Fiyat - price veya bid_fee
        const price = c.price || c.bid_fee || 0;
        
        const customerData = {
          id: uuidv4(),
          user_id: userId || req.user.id,
          category_id: categoryId,
          business_name: c.name || 'Bilinmeyen',
          contact_name: c.authorized_person || '',
          email: c.email || '',
          phone: c.phone || '',
          address: '',
          city: '',
          ...
          tax_number: c.tax_number ? String(c.tax_number) : '',
          price: parseFloat(price) || 0,
          is_invoiced: c.tax_included === 1 || c.tax_included === true,
          scene_count: parseInt(c.scene_count) || 1,
          status: statusId,
          notes: c.note || '',
          is_deleted: c.deleted_at ? true : false,
        ...
        if (error) {
          results.failed++;
          results.errors.push({ name: c.name, error: error.message });
        } else {
          results.success++;
        }
      } catch (err) {
    ...
    res.status(500).json({ error: 'Import hatası: ' + err.message });
  }
});

// CATEGORIES & STATUSES
app.get('/api/categories', auth, (req, res) => {
  // Admin tüm kategorileri görebilir, diğerleri sadece ilk 22'yi
  if (req.user.role === 'admin') {
    res.json(CATEGORIES);
  } else {
    res.json(CATEGORIES.filter(c => c.id <= 22));
  }
});
app.get('/api/statuses', auth, (req, res) => res.json(STATUSES));

// Müşterilerdeki benzersiz şehirleri getir
app.get('/api/cities', auth, async (req, res) => {
  ...
  try {
    ...
    const cities = [...new Set((data || []).map(c => c.city).filter(c => c && c.trim()))];
...
// Debug endpoint - kullanıcının allowed_cities değerini görmek için
app.get('/api/debug/user/:id', auth, adminOnly, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('users')
      .select('id, username, full_name, role, team_id, allowed_cities')
      .eq('id', req.params.id)
      .single();
    if (error) return res.status(500).json({ error: error.message });
    res.json({
      user: data,
      allowed_cities_type: typeof data?.allowed_cities,
      allowed_cities_value: data?.allowed_cities,
      is_array: Array.isArray(data?.allowed_cities)
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/customers', auth, async (req, res) => {
  if (!supabase) return res.json([]);
  try {
    // Tüm kullanıcıları çek (is_deleted kontrolü olmadan)
    ...
    if (req.user.role === 'sales') {
      // Satış: Sadece kendi kayıtları
      filtered = filtered.filter(c => c.user_id === req.user.id);
    } else if (req.user.role === 'manager') {
      // Bölge Yöneticisi: Doğrudan veritabanından güncel user bilgisini al
      const currentUser = allUsers.find(u => u.id === req.user.id);
      
      console.log('========================================');
      console.log('MANAGER FILTER - User:', currentUser?.full_name);
      console.log('MANAGER FILTER - Raw allowed_cities:', currentUser?.allowed_cities);
      console.log('MANAGER FILTER - Type:', typeof currentUser?.allowed_cities);
      
      // allowed_cities'i al ve parse et (JSON string olarak saklandı)
      let allowedCities = [];
      const rawCities = currentUser?.allowed_cities;
      
      if (rawCities) {
        if (Array.isArray(rawCities)) {
          // Zaten array (eski format)
          allowedCities = rawCities;
        } else if (typeof rawCities === 'string') {
          // String - JSON parse dene
          try {
            const parsed = JSON.parse(rawCities);
            allowedCities = Array.isArray(parsed) ? parsed : [parsed];
          } catch(e) {
            // JSON parse başarısız - PostgreSQL array formatı olabilir
            if (rawCities.startsWith('{') && rawCities.endsWith('}')) {
              allowedCities = rawCities.slice(1, -1).split(',').map(s => s.trim().replace(/"/g, ''));
            } else {
              // Tek şehir
              allowedCities = [rawCities];
            }
      ...
      allowedCities = allowedCities.filter(c => c && String(c).trim());
      
      console.log('MANAGER FILTER - Parsed cities:', allowedCities);
      ...
      // Ekip üyelerinin ID'leri
      const teamUserIds = allUsers.filter(u => u.team_id === currentUser?.team_id).map(u => u.id);
      console.log('MANAGER FILTER - Team IDs:', teamUserIds);
      console.log('MANAGER FILTER - Total customers:', filtered.length);
      
      // Müşterilerdeki şehirleri logla
      const customerCities = [...new Set(filtered.map(c => c.city).filter(c => c))];
      console.log('MANAGER FILTER - Customer cities (sample):', customerCities.slice(0, 10));
      
      filtered = filtered.filter(c => {
        // 1. Kendi ekibinin kayıtları
        if (teamUserIds.includes(c.user_id)) {
          return true;
        }
        
        // 2. İzin verilen şehirlerdeki kayıtlar
        if (allowedCities.length > 0 && c.city) {
          const customerCity = normalizeTurkish(c.city);
          const match = allowedCities.some(city => {
            const allowedCity = normalizeTurkish(city);
            // Esnek eşleşme: tam eşleşme veya içerme
            const isMatch = customerCity === allowedCity || 
                   customerCity.includes(allowedCity) || 
                   allowedCity.includes(customerCity);
            return isMatch;
          });
          return match;
        }
      ...
      console.log('MANAGER FILTER - Filtered count:', filtered.length);
      console.log('========================================');
    }
    // supervisor ve admin: Tüm kayıtları görür (filter yok)
    
    res.json(filtered.map(c => ({ 
      ...c, 
      user_name: allUsers.find(u => u.id === c.user_id)?.full_name,
      user_photo: allUsers.find(u => u.id === c.user_id)?.profile_photo,
...
// Silinen müşteriler - ÖNEMLİ: Bu endpoint :id'den ÖNCE tanımlanmalı!
app.get('/api/customers/deleted', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    // 30 günden eski silinenleri filtrele
    ...
    res.json(filtered.map(c => ({ 
      ...
      user_photo: users.find(u => u.id === c.user_id)?.profile_photo,
      category: CATEGORIES.find(cat => cat.id === c.category_id), 
      status_info: STATUSES.find(s => s.id === c.status) 
    })));
...
app.post('/api/customers', auth, async (req, res) => {
  try {
    ...
    // Kategori yetki kontrolü: 22'den büyük kategoriler sadece admin için
    const catId = parseInt(category_id);
    if (catId > 22 && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Bu kategoriyi seçme yetkiniz yok' });
    }
    ...
    const customer = await insert('customers', { user_id: req.user.id, category_id: catId, business_name, place_id: place_id || null, address: address || null, city: city || null, district: district || null, contact_name: contact_name || null, phone: phone || null, email: email || null, tax_number: tax_number || null, price: priceVal, price_without_vat: Math.round(priceWithoutVat * 100) / 100, vat_amount: Math.round(vatAmount * 100) / 100, is_invoiced: is_invoiced || false, scene_count: parseInt(scene_count) || 1, status: 1, notes: notes || null });
...
app.put('/api/customers/:id', auth, async (req, res) => {
  try {
    ...
    let priceVal = parseFloat(price) || 0, priceWithoutVat = priceVal, vatAmount = 0;
    if (is_invoiced && priceVal > 0) { priceWithoutVat = priceVal / 1.20; vatAmount = priceVal - priceWithoutVat; }
    const updated = await update('customers', req.params.id, { category_id: catId, business_name, place_id: place_id || null, address: address || null, city: city || null, district: district || null, contact_name: contact_name || null, phone: phone || null, email: email || null, tax_number: tax_number || null, price: priceVal, price_without_vat: Math.round(priceWithoutVat * 100) / 100, vat_amount: Math.round(vatAmount * 100) / 100, is_invoiced: is_invoiced || false, scene_count: parseInt(scene_count) || 1, status: parseInt(status) || c.status, notes: notes || null });
...
app.post('/api/emails/send', auth, async (req, res) => {
  try {
    // Günlük mail limiti kontrolü (admin hariç, günde 20 mail)
    if (req.user.role !== 'admin') {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const allEmails = await getAll('emails');
      const todayEmails = allEmails.filter(e => 
        e.user_id === req.user.id && 
        new Date(e.sent_at) >= today
      );
      if (todayEmails.length >= 20) {
        return res.status(429).json({ error: 'Günlük mail limitine ulaştınız (20 mail). Yarın tekrar deneyebilirsiniz.' });
      }
    }
