...
function initializeData() {
  ...
  let templates = readDB('templates');
  if (templates.length === 0) {
    const adminUser = users.find(u => u.username === 'burakkaan48');
    templates.push({
      id: uuidv4(),
...
app.get('/api/users', authenticateToken, requireAdmin, (req, res) => {
  const users = readDB('users').map(u => ({ id: u.id, username: u.username, full_name: u.full_name, role: u.role, created_at: u.created_at, is_active: u.is_active }));
  res.json(users);
});
...
app.post('/api/users', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const newUser = {
      id: uuidv4(), username, password: bcrypt.hashSync(password, 10), full_name, role: 'user',
      created_at: new Date().toISOString(), is_active: 1
    };
    users.push(newUser);
    writeDB('users', users);
    res.json({ id: newUser.id, username, full_name, role: 'user' });
  } catch (error) {
...
app.put('/api/users/:id', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    if (is_active !== undefined) users[idx].is_active = is_active ? 1 : 0;
    writeDB('users', users);
...
app.get('/api/templates', authenticateToken, (req, res) => {
  const templates = readDB('templates').filter(t => t.is_active);
  const users = readDB('users');
  templates.forEach(t => {
    const creator = users.find(u => u.id === t.created_by);
    t.created_by_name = creator?.full_name;
  });
  res.json(templates);
});

app.get('/api/templates/:id', authenticateToken, (req, res) => {
  const template = readDB('templates').find(t => t.id === req.params.id);
...
app.post('/api/templates', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    let templates = readDB('templates');
    const newTemplate = {
      id: uuidv4(), name, subject, body, variables: variables || [],
...
app.put('/api/templates/:id', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    if (idx === -1) return res.status(404).json({ error: 'Taslak bulunamadı' });
    
    templates[idx] = { ...templates[idx], name, subject, body, variables: variables || [], updated_at: new Date().toISOString() };
    writeDB('templates', templates);
    res.json({ success: true });
  } catch (error) {
...
app.get('/api/emails/:id', authenticateToken, (req, res) => {
  ...
  const sender = users.find(u => u.id === email.user_id);
  const template = templates.find(t => t.id === email.template_id);
  email.sender_name = sender?.full_name;
...
app.post('/api/emails/send', authenticateToken, async (req, res) => {
  try {
    ...
    const newEmail = {
      id: emailId, user_id: req.user.id, template_id, recipient_email, recipient_name,
      subject, body, variables_used: variables, status: 'sending', sent_at: new Date().toISOString()
    };
    ...
    if (!transporter) {
      ...
      if (idx !== -1) { emails[idx].status = 'failed'; emails[idx].error_message = 'Gmail yapılandırılmamış'; writeDB('emails', emails); }
      return res.status(500).json({ error: 'Gmail yapılandırılmamış. Lütfen admin ile iletişime geçin.' });
    }
    
    try {
      await transporter.sendMail({ from: `"G360 AI" <${GMAIL_USER}>`, to: recipient_email, subject, html: body });
      emails = readDB('emails');
      const idx = emails.findIndex(e => e.id === emailId);
      if (idx !== -1) { emails[idx].status = 'sent'; writeDB('emails', emails); }
      res.json({ success: true, message: 'Mail başarıyla gönderildi', emailId });
    } catch (emailError) {
      emails = readDB('emails');
      const idx = emails.findIndex(e => e.id === emailId);
      if (idx !== -1) { emails[idx].status = 'failed'; emails[idx].error_message = emailError.message; writeDB('emails', emails); }
...
app.get('/api/stats', authenticateToken, requireAdmin, (req, res) => {
  ...
  res.json({
    ...
    recentEmails: emails.sort((a, b) => new Date(b.sent_at) - new Date(a.sent_at)).slice(0, 5)
  });
});
  ...
  <script>
      ...
      return \`
        <div class="max-w-4xl mx-auto space-y-6 animate-fadeIn">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="glass rounded-xl p-6">
              <form id="sendForm" class="space-y-5">
                <div>
                  ...
                  <select id="templateSelect" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200 focus:outline-none focus:border-primary-500"><option>Yükleniyor...</option></select>
                </div>
            ...
            <div class="glass rounded-xl p-6">
              <h3 class="font-semibold text-slate-200 mb-4">📧 Mail Önizlemesi</h3>
              <div id="previewContainer" class="bg-white rounded-lg overflow-hidden max-h-96 overflow-y-auto"><div class="p-4 text-gray-500">Taslak seçin...</div></div>
            </div>
          </div>
        </div>
      \`;
        ...
        <div class="space-y-6 animate-fadeIn">
          ...
          <div class="glass rounded-xl overflow-hidden">
            <table class="w-full">
              ...
              <tbody id="emailsTable"><tr><td colspan="5" class="py-8 text-center text-slate-400">Yükleniyor...</td></tr></tbody>
            </table>
          </div>
        </div>
      \`;
      ...
      return \`
        <div class="space-y-6 animate-fadeIn">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <button onclick="showUserModal()" class="btn-primary text-white px-5 py-2.5 rounded-lg flex items-center gap-2 w-fit">+ Yeni Kullanıcı</button>
          </div>
          <div class="glass rounded-xl overflow-hidden">
            <table class="w-full">
              <thead>
              ...
              <tbody id="usersTable"><tr><td colspan="5" class="py-8 text-center text-slate-400">Yükleniyor...</td></tr></tbody>
            </table>
          </div>
        </div>
        <div id="userModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
          <div class="glass rounded-xl p-6 max-w-md w-full mx-4">
            <h3 class="text-lg font-semibold text-slate-100 mb-4">Yeni Kullanıcı</h3>
            <form id="userForm" class="space-y-4">
              <div><label class="block text-sm text-slate-300 mb-2">Kullanıcı Adı *</label><input type="text" id="newUsername" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200" required></div>
              ...
              <div><label class="block text-sm text-slate-300 mb-2">Şifre *</label><input type="password" id="newPassword" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200" required></div>
              <div class="flex gap-3 pt-2">
                <button type="button" onclick="hideUserModal()" class="flex-1 bg-slate-700 text-slate-200 py-3 rounded-lg">İptal</button>
      ...
      return \`
        <div class="space-y-6 animate-fadeIn">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <button onclick="showTemplateModal()" class="btn-primary text-white px-5 py-2.5 rounded-lg flex items-center gap-2 w-fit">+ Yeni Taslak</button>
          </div>
          <div id="templatesGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"><div class="glass rounded-xl p-6 text-slate-400">Yükleniyor...</div></div>
        </div>
        <div id="templateModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden overflow-y-auto py-8">
          <div class="glass rounded-xl p-6 max-w-2xl w-full mx-4">
            ...
            <form id="templateForm" class="space-y-4">
              <div><label class="block text-sm text-slate-300 mb-2">Taslak Adı *</label><input type="text" id="tplName" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200" required></div>
              <div><label class="block text-sm text-slate-300 mb-2">Mail Konusu *</label><input type="text" id="tplSubject" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200" placeholder="Örn: Teklif - {isletme_adi}" required></div>
              <div><label class="block text-sm text-slate-300 mb-2">Değişkenler (virgülle ayırın)</label><input type="text" id="tplVariables" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200" placeholder="alici_adi, isletme_adi, fiyat"></div>
              <div><label class="block text-sm text-slate-300 mb-2">Mail İçeriği (HTML) *</label><textarea id="tplBody" rows="10" class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-slate-200 font-mono text-sm" required></textarea></div>
              <div class="flex gap-3 pt-2">
                <button type="button" onclick="hideTemplateModal()" class="flex-1 bg-slate-700 text-slate-200 py-3 rounded-lg">İptal</button>
                <button type="submit" class="flex-1 btn-primary text-white py-3 rounded-lg">Kaydet</button>
              </div>
            </form>
          </div>
    ...
    function renderLayout(content) {
      const navItems = [
        ...
        { id: 'send', icon: 'M12 19l9 2-9-18-9 18 9-2zm0 0v-8', label: 'Mail Gönder' },
        { id: 'emails', icon: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', label: 'Mailler' }
      ];
      const adminItems = [
        { id: 'templates', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', label: 'Taslaklar' },
        { id: 'users', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', label: 'Kullanıcılar' }
      ];
          ...
          <aside class="w-64 bg-slate-900/95 border-r border-slate-800 hidden lg:flex flex-col">
            <div class="p-6 border-b border-slate-800">
              <div class="flex items-center gap-3">
                ...
                <div><h1 class="text-xl font-bold gradient-text">G360 AI</h1><p class="text-xs text-slate-500">Mail Sistemi</p></div>
              </div>
            </div>
            <nav class="flex-1 p-4 space-y-1">
              <p class="text-xs font-semibold text-slate-500 uppercase px-4 mb-2">Ana Menü</p>
              \${navItems.map(item => \`<button onclick="navigate('\${item.id}')" class="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all \${currentPage === item.id ? 'bg-gradient-to-r from-primary-600/20 to-secondary-600/20 text-primary-400' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="\${item.icon}"></path></svg><span>\${item.label}</span></button>\`).join('')}
              \${isAdmin() ? \`<p class="text-xs font-semibold text-slate-500 uppercase px-4 mt-6 mb-2">Yönetim</p>\${adminItems.map(item => \`<button onclick="navigate('\${item.id}')" class="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all \${currentPage === item.id ? 'bg-gradient-to-r from-primary-600/20 to-secondary-600/20 text-primary-400' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="\${item.icon}"></path></svg><span>\${item.label}</span></button>\`).join('')}\` : ''}
            </nav>
            <div class="p-4 border-t border-slate-800">
              <div class="flex items-center gap-3 px-4 py-3 rounded-lg bg-slate-800/50">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-secondary-500 flex items-center justify-center text-white font-bold">\${currentUser?.full_name?.charAt(0).toUpperCase()}</div>
                <div class="flex-1"><p class="font-medium text-slate-200 truncate">\${currentUser?.full_name}</p><p class="text-xs text-slate-500">\${isAdmin() ? 'Yönetici' : 'Kullanıcı'}</p></div>
                <button onclick="logout()" class="p-2 text-slate-400 hover:text-red-400" title="Çıkış"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg></button>
              </div>
            </div>
          </aside>
    ...
    async function loadPageData() {
      try {
        if (currentPage === 'dashboard') {
          ...
          if (!isAdmin()) { stats.totalEmails = emails.length; stats.sentEmails = emails.filter(e => e.status === 'sent').length; stats.failedEmails = emails.filter(e => e.status === 'failed').length; }
          ...
          document.getElementById('recentEmails').innerHTML = recent.length ? recent.map(e => \`<div class="flex items-center justify-between py-3 border-b border-slate-700 last:border-0"><div><p class="text-slate-200">\${e.recipient_name || e.recipient_email}</p><p class="text-xs text-slate-500">\${e.subject}</p></div><span class="px-2 py-1 rounded-full text-xs \${e.status === 'sent' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}">\${e.status === 'sent' ? 'Gönderildi' : 'Başarısız'}</span></div>\`).join('') : '<p class="text-slate-500 text-center py-4">Henüz mail gönderilmemiş</p>';
        }
        ...
        if (currentPage === 'emails') {
          ...
          document.getElementById('emailsTable').innerHTML = emails.length ? emails.map(e => \`<tr class="border-b border-slate-800 hover:bg-slate-800/30"><td class="py-4 px-6"><p class="text-slate-200">\${e.recipient_name || '-'}</p><p class="text-xs text-slate-500">\${e.recipient_email}</p></td>\${isAdmin() ? \`<td class="py-4 px-6 text-slate-300">\${e.sender_name || '-'}</td>\` : ''}<td class="py-4 px-6 text-slate-300 max-w-xs truncate">\${e.subject}</td><td class="py-4 px-6"><span class="px-2 py-1 rounded-full text-xs \${e.status === 'sent' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}">\${e.status === 'sent' ? 'Gönderildi' : 'Başarısız'}</span></td><td class="py-4 px-6 text-slate-400 text-sm">\${new Date(e.sent_at).toLocaleString('tr-TR')}</td></tr>\`).join('') : '<tr><td colspan="5" class="py-8 text-center text-slate-400">Henüz mail gönderilmemiş</td></tr>';
        }
        
        if (currentPage === 'users') {
          const users = await api('/api/users');
          document.getElementById('userCount').textContent = users.filter(u => u.role !== 'admin').length + ' kullanıcı';
          document.getElementById('usersTable').innerHTML = users.map(u => \`<tr class="border-b border-slate-800 hover:bg-slate-800/30"><td class="py-4 px-6"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-full bg-gradient-to-br \${u.role === 'admin' ? 'from-amber-500 to-orange-500' : 'from-primary-500 to-secondary-500'} flex items-center justify-center text-white font-bold">\${u.full_name?.charAt(0).toUpperCase()}</div><span class="text-slate-200">\${u.full_name}</span></div></td><td class="py-4 px-6 text-slate-400">\${u.username}</td><td class="py-4 px-6"><span class="px-2 py-1 rounded-full text-xs \${u.role === 'admin' ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-700 text-slate-300'}">\${u.role === 'admin' ? 'Admin' : 'Kullanıcı'}</span></td><td class="py-4 px-6"><span class="text-sm \${u.is_active ? 'text-emerald-400' : 'text-slate-500'}">\${u.is_active ? 'Aktif' : 'Pasif'}</span></td><td class="py-4 px-6 text-right">\${u.role !== 'admin' ? \`<button onclick="deleteUser('\${u.id}')" class="text-red-400 hover:text-red-300 text-sm">Sil</button>\` : ''}</td></tr>\`).join('');
        ...
        if (currentPage === 'templates') {
          ...
          document.getElementById('templatesGrid').innerHTML = templates.length ? templates.map(t => \`<div class="glass rounded-xl p-6"><div class="flex items-start justify-between mb-3"><div class="p-2 rounded-lg bg-amber-500/20 text-amber-400"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg></div><button onclick="deleteTemplate('\${t.id}')" class="text-slate-400 hover:text-red-400"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div><h3 class="font-semibold text-slate-200 mb-2">\${t.name}</h3><p class="text-sm text-slate-400 mb-4 truncate">\${t.subject}</p><div class="flex flex-wrap gap-1">\${(t.variables || []).slice(0, 3).map(v => \`<span class="px-2 py-1 rounded bg-slate-700/50 text-xs text-slate-400">\${v}</span>\`).join('')}</div></div>\`).join('') : '<div class="glass rounded-xl p-6 text-slate-400">Henüz taslak yok</div>';
    ...
    function updateSendForm() {
      ...
      if (!template) return;
      document.getElementById('variablesContainer').innerHTML = template.variables?.length ? \`<div class="space-y-3"><label class="block text-sm font-medium text-slate-300">Değişkenler</label>\${template.variables.map(v => \`<div><label class="block text-xs text-slate-400 mb-1">\${v}</label><input type="text" data-var="\${v}" class="var-input w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-2 text-slate-200 text-sm focus:outline-none focus:border-primary-500" placeholder="\${v} girin..."></div>\`).join('')}</div>\` : '';
      document.querySelectorAll('.var-input').forEach(input => input.addEventListener('input', updatePreview));
      updatePreview();
    }
    ...
    function updatePreview() {
      ...
      document.querySelectorAll('.var-input').forEach(input => {
        ...
        body = body.replace(new RegExp(\`{\${varName}}\`, 'g'), input.value || \`[\${varName}]\`);
      });
    ...
    async function handleLogin(e) {
      e.preventDefault();
      try {
        const data = await api('/api/auth/login', { method: 'POST', body: JSON.stringify({ username: document.getElementById('username').value, password: document.getElementById('password').value }) });
    ...
    async function handleSendEmail(e) {
      e.preventDefault();
      const variables = {};
      document.querySelectorAll('.var-input').forEach(input => { variables[input.dataset.var] = input.value; });
      try {
        await api('/api/emails/send', { method: 'POST', body: JSON.stringify({ template_id: document.getElementById('templateSelect').value, recipient_email: document.getElementById('recipientEmail').value, recipient_name: document.getElementById('recipientName').value, variables }) });
    ...
    async function handleAddUser(e) {
      ...
      try {
        ...
        hideUserModal(); loadPageData();
      } catch (error) { showToast(error.message, 'error'); }
    }
    
    async function deleteUser(id) {
      if (!confirm('Bu kullanıcıyı silmek istediğinize emin misiniz?')) return;
      try { await api('/api/users/' + id, { method: 'DELETE' }); showToast('Kullanıcı silindi'); loadPageData(); } catch (error) { showToast(error.message, 'error'); }
    }
    ...
    async function handleAddTemplate(e) {
      ...
      try {
        ...
        hideTemplateModal(); loadPageData();
      } catch (error) { showToast(error.message, 'error'); }
    }
    
    async function deleteTemplate(id) {
      if (!confirm('Bu taslağı silmek istediğinize emin misiniz?')) return;
      try { await api('/api/templates/' + id, { method: 'DELETE' }); showToast('Taslak silindi'); loadPageData(); } catch (error) { showToast(error.message, 'error'); }
    }
...
app.get('/health', (req, res) => res.json({ status: 'ok', gmail: GMAIL_USER, configured: !!GMAIL_APP_PASSWORD }));
